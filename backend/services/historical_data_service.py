...
# 2. /app/data (Docker mount)
# 3. /data (альтернативный Docker mount)
# 4. ../../data относительно этого файла (локальная разработка)
_env_data_path = os.environ.get("HISTORICAL_DATA_PATH")
if _env_data_path:
    HISTORICAL_DATA_PATH = Path(_env_data_path)
elif Path("/app/data").exists():
    HISTORICAL_DATA_PATH = Path("/app/data")
...
class HistoricalDataAnalyzer:
    ...
    def load_historical_data(self) -> bool:
        ...
        try:
            ...
            if buy_file.exists():
                ...
                if self.buy_data:
                    logger.info(
                        f"Загружено покупок для сервера {self.server_id}: {len(self.buy_data)} предметов"
                    )
                else:
                    logger.warning(f"Не удалось загрузить файл покупок: {buy_file}")
            else:
                logger.warning(
                    f"Файл покупок не найден: {buy_file} - исторические данные для покупок будут недоступны"
                )

            # Загрузка данных о продажах
            sell_file = (
                HISTORICAL_DATA_PATH / f"info_users_sell_{self.server_name}.json"
            )
            if sell_file.exists():
                # Пробуем несколько кодировок для максимальной совместимости
                ...
                if self.sell_data:
                    logger.info(
                        f"Загружено продаж для сервера {self.server_id}: {len(self.sell_data)} предметов"
                    )
                else:
                    logger.warning(f"Не удалось загрузить файл продаж: {sell_file}")
            else:
                logger.warning(
                    f"Файл продаж не найден: {sell_file} - исторические данные для продаж будут недоступны"
                )
            ...
            logger.error(
                f"Ошибка парсинга JSON исторических данных для сервера {self.server_id}: {e}"
            )
            return False
        except FileNotFoundError as e:
            logger.error(
                f"Файл исторических данных не найден для сервера {self.server_id}: {e}"
            )
            return False
        except Exception as e:
            logger.error(
                f"Неожиданная ошибка при загрузке исторических данных для сервера {self.server_id}: {e}"
            )
            return False
    ...
    def _load_json_with_fallback(self, file_path: Path) -> Optional[Dict]:
        ...
        for encoding in encodings:
            try:
                with open(file_path, "r", encoding=encoding) as f:
                    ...
                    if encoding != "utf-8":
                        logger.warning(
                            f"Файл {file_path} загружен с fallback кодировкой {encoding}"
                        )
                    return data
        ...
        logger.error(
            f"Не удалось прочитать файл {file_path} ни в одной из кодировок: {encodings}"
        )
        return None
    ...
    @cache_result(ttl=900, key_prefix="item_history", entity_type="marketplace")  # Cache for 15 minutes
    async def get_item_history(self, item_name: str, mode: str = "buy") -> List[Dict]:
        """
    ...
    @cache_result(ttl=600, key_prefix="price_trend", entity_type="marketplace")  # Cache for 10 minutes
    async def calculate_price_trend(
        self, item_name: str, mode: str = "buy", days: int = 7
    ) -> Dict[str, Any]:
        """
        ...
        """
        history = await self.get_item_history(item_name, mode)
    ...
    @cache_result(ttl=1800, key_prefix="liquidity_score", entity_type="marketplace")  # Cache for 30 minutes
    async def calculate_liquidity_score(
        self, item_name: str, mode: str = "buy", days: int = 30
    ) -> float:
        """
        ...
        for h in recent_history:
            try:
                trade_date = datetime.strptime(h["date"], "%Y-%m-%d").replace(
                    tzinfo=timezone.utc
                )
        ...
        consistency_score = (
            (days_with_trades / max_possible_days) * 100
            if max_possible_days > 0
            else 0.0
        )

        # 5. Спред (10%) - рассчитываем на основе разницы buy/sell
        spread_data = await self.calculate_spread(item_name, days=min(days, 7))
    ...
    @cache_result(ttl=1200, key_prefix="spread_calc", entity_type="marketplace")  # Cache for 20 minutes
    async def calculate_spread(self, item_name: str, days: int = 7) -> Dict[str, Any]:
        """
        ...
        buy_history = await self.get_item_history(item_name, "buy")
        sell_history = await self.get_item_history(item_name, "sell")
        ...
        buy_recent = [
            h for h in buy_history if h["date"] >= cutoff_date and h["price"] > 0
        ]
        sell_recent = [
            h for h in sell_history if h["date"] >= cutoff_date and h["price"] > 0
        ]
        ...
        spread_percent = (
            (spread_absolute / buy_avg_price * 100) if buy_avg_price > 0 else 0
        )
    ...
    async def get_24h_activity(self, item_name: str, mode: str = "buy") -> Dict[str, Any]:
        """
