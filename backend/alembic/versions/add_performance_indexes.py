...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_users_created_at", "users", ["created_at"], unique=False, if_not_exists=True
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_global_settings_updated_at", table_name="global_settings", if_exists=True
    )
    ...
    op.drop_index(
        "ix_audit_logs_status_created", table_name="audit_logs", if_exists=True
    )
    op.drop_index("ix_price_alerts_server", table_name="price_alerts", if_exists=True)
    op.drop_index(
        "ix_price_alerts_active_triggered", table_name="price_alerts", if_exists=True
    )
    op.drop_index(
        "ix_favorite_items_mode_server", table_name="favorite_items", if_exists=True
    )
    op.drop_index(
        "ix_favorite_items_active", table_name="favorite_items", if_exists=True
    )
    op.drop_index(
        "ix_config_history_server_mode", table_name="config_history", if_exists=True
    )
